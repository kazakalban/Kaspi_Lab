...
@dag(
    ...
    tags=["download_json", "wait_for_file", "create_table", "parse_json", "load_to_postgres"],
    catchup=False,
)
def etl_create_comments_and_posts_table():
    ...
    def wait_for_file():
        ...
        for file_path in files:
            if os.path.exists(file_path):
                ...
                print(f"Файла нет: {file_path}")
                all_exist = False
        return all_exist

    @task
    def create_stage_tables():
        """Создает промежуточные (staging) таблицы в Postgres, если они не существуют."""
    ...
    def load_stg_to_postgres(result_parse_json: dict):
        """Загружает данные из JSON в промежуточные таблицы (stg_comments, stg_posts)."""
        postgres_hook = PostgresHook(postgres_conn_id="pg_conn")
        conn = postgres_hook.get_conn()
        cursor = conn.cursor()
    ...
    def create_clean_tables():
        """Создает аналитические таблицы (fact, dq, users) для чистых данных."""
        postgres_hook = PostgresHook(postgres_conn_id="pg_conn")
        conn = postgres_hook.get_conn()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def aggregate_all(result_parse_json: dict):
        """Объединяет данные posts и comments, формирует агрегаты для анализа."""
        ...
        # Формируем таблицу fact_post_comments
        fact_post_comments = []
        for post in posts:
            post_id = post["id"]
        ...
        # Формируем dq_missed_post_comments (комментарии без существующих постов)
        ...
        # Подсчитываем количество постов по каждому userId
        # Cколько постов создал каждый пользователь, и делает из этого аккуратный список словарей
    ...
    def load_fact_to_postgres(aggregated: dict):
        """Загружает агрегированные данные в таблицы fact_post_comments, dq_missed_post_comments и users."""
        postgres_hook = PostgresHook(postgres_conn_id="pg_conn")
        conn = postgres_hook.get_conn()
        cursor = conn.cursor()

        # Заполняем таблицу fact_post_comments
        ...
        # Заполняем таблицу dq_missed_post_comments
